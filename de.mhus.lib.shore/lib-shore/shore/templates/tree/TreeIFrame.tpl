...
<%
		...
		<t:options>
		shoreTree.getOptions().put( "{v:key}", "{v:value}" );
		</t:options>
		...
		if ( shoreTreeDoc != null ) {
          ...
          // events
  		  ...
  		  if ( shoreTreeDoc.getReloadPage() ) {
  		  	...
  		  	out.println( "window.parent.location=\"{v:parentFile_pp}\";" );
  		  	out.println( "</script>" );
  		  	return;
  		  }
		  ...
		  while ( shoreTreeDoc.next() ) {
			...
			out.print( "<a name=\"shoreTree_{v:name_p}_" + shoreTreeDoc.getNodeReference() + "\">" );
			...
			for ( int i = 0; i < shoreTreeDoc.getLevels(); i++ ) {
				...
				if ( i == shoreTreeDoc.getLevels()-1 && ! shoreNode.isLeaf() ) {
					out.print( "<a style=\"{v:style_pp}\" href=\"{v:file_url}?shoreTree_{v:name_url}_exp=" + shoreTreeDoc.getNodeReference() + r + "#shoreTree_{v:name_url}_" + shoreTreeDoc.getNodeReference() + "\">" );
				}

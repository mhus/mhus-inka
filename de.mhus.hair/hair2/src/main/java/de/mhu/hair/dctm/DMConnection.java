...
public class DMConnection implements Api {
	...
	public DMConnection(String pUser, String pPass, String pDocBase)
		...
		try {
			...
			userName = pUser;
			docBase = pDocBase;
			String userPass = APassword.decode(pPass);
	...
	public IDfSysObject getExistingObject(String path, String lang)
			throws DfException {
		...
		if (pos < 0) {
			dql = "SELECT r_object_id FROM dm_cabinet WHERE object_name='"
					+ ASql.escape(path) + " AND language_code='"
					+ ASql.escape(lang) + "'";
		} else {
			String folder = path.substring(0, pos);
			String name = path.substring(pos + 1);
			dql = "SELECT r_object_id FROM dm_sysobject WHERE FOLDER('"
					+ ASql.escape(folder) + "') AND object_name='"
					+ ASql.escape(name) + "' AND language_code='"
					+ ASql.escape(lang) + "'";
		}

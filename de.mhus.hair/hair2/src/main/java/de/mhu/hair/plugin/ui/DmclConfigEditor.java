...
public class DmclConfigEditor {
	...
	private File dmcl;
	private TreeMap<String, String> dmclTemplates;
	...
	public DmclConfigEditor() throws Exception {
		this(null);
	}
	
	//$hide>>$
	public DmclConfigEditor(Component parentComponent) throws Exception {
	...
	private void reloadConfigs() {
		...
		for ( String cname : dmclTemplates.keySet() )
	...
	protected boolean init() {
		...
		if ( dmShared == null ) {
			...
			try {
				...
				if ( sharedString != null ) sharedString=sharedString;
			} catch ( Exception e ) {
				e.printStackTrace();
			}
			if ( sharedString == null ) {
				sharedString = System.getenv("DOCUMENTUM");			
			}
			if ( sharedString == null ) {
				//JOptionPane.showMessageDialog(null, "$DOCUMENTUM_SHARED not set" );
				JFileChooser fs = new JFileChooser();
				fs.setDialogTitle("Choose Documentum Directory");
				fs.setMultiSelectionEnabled(false);
				fs.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
				if ( fs.showOpenDialog(null) != JFileChooser.APPROVE_OPTION )
					return false;
				
				dmShared = fs.getSelectedFile();
				
			} else {
				dmShared = new File( sharedString );
			}
		}
		
		if ( ! dmShared.exists() ) {
			JOptionPane.showMessageDialog(null, "$DOCUMENTUM not extists" );
			return false;
		}
		
		if ( ! dmShared.isDirectory() ) {
			JOptionPane.showMessageDialog(null, "$DOCUMENTUM is not a directory" );
			return false;
		}
		...
		if ( System.getenv("WINDIR") != null ) {
			// windows
			dmcl = new File(System.getenv("WINDIR") + "/dmcl.ini" );
			dmHome = new File(System.getenv("WINDIR"));
		} else {
			dmHome = new File(dmShared, ".." );
			dmcl = new File( dmHome, "dmcl.ini" );			
		}
				
		if ( !dmcl.exists() ) {
			JOptionPane.showMessageDialog(null, "$DOCUMENTUM/dmcl.ini not extists\nShared: " + dmShared + "\nHome: " + dmHome );
			return false;
		}
		
		// list of dmcl templates
		
		String[] list = dmHome.list();
		dmclTemplates = new TreeMap<String,String>();
		for ( String item : list )
			if ( item.startsWith( "dmcl-") && item.endsWith( ".ini" ) )
				dmclTemplates.put(item.substring(5,item.lastIndexOf('.')), item );
	...
	protected JPanel initUI() {
		...
		mainPanelLayout.setVerticalGroup(mainPanelLayout.createSequentialGroup()
			...
			.addComponent(getLDocbroker(), GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
			.addPreferredGap(LayoutStyle.ComponentPlacement.RELATED)
			.addComponent(getJScrollPane1(), 0, 271, Short.MAX_VALUE)
			.addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED)
			.addGroup(mainPanelLayout.createParallelGroup(GroupLayout.Alignment.BASELINE)
		...
		mainPanelLayout.setHorizontalGroup(mainPanelLayout.createSequentialGroup()
			...
			.addGroup(mainPanelLayout.createParallelGroup()
			    ...
			    .addComponent(getLDocbroker(), GroupLayout.Alignment.LEADING, 0, 382, Short.MAX_VALUE)
	...
	public static int show(Component parent) throws Exception {
		...
		DmclConfigEditor editor = new DmclConfigEditor(dialog);
		...
		dialog.setTitle("DMCL Config Editor");
	...
	private JTextArea getTConfigValue() {
		if(tConfigValue == null) {
			...
			//tConfigValue.setPreferredSize(new java.awt.Dimension(379, 230));
	...
	private void bSelectActionPerformed(ActionEvent evt) {
		...
		if ( currentFile.equals( dmcl ) ) {
			result = NOT_CHANGED;
		} else {
			if ( ! AFile.copyFile(currentFile, dmcl) )
				result = ERROR;
			result = APPROVED;	
		}
		if ( parentComponent != null ) parentComponent.setVisible(false);
...
//		}
		...
		if ( DEFAULT_ITEM.equals(item) ) {
			...
			currentFile = new File(dmHome,dmclTemplates.get(item));
		...
		if ( currentFile != null && currentFile.exists() ) {
			...
			try {
				...
				p.load(is);
				is.close();
				lDocbroker.setText( p.getProperty("host") + ":" + p.getProperty("port"));
			} catch ( Exception e ) {
				e.printStackTrace();
			}
	...
	private void bNewConfigActionPerformed(ActionEvent evt) {
		String title = JOptionPane.showInputDialog( mainPanel, "Insert new config title", "Create DMCL Config", JOptionPane.INFORMATION_MESSAGE);
		if ( ! AFile.copyFile(currentFile, new File(dmHome, "dmcl-" + title+".ini") ) ) {
			JOptionPane.showMessageDialog(mainPanel, "Can't create configuration.", "Create DMCL Config", JOptionPane.ERROR_MESSAGE);
		} else {
			init();
			reloadConfigs();
	...
	private void bDeleteConfigActionPerformed(ActionEvent evt) {
		...
		new File( dmHome, dmclTemplates.get(item)).delete();
		init();
		reloadConfigs();
	}

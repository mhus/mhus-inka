...
public class PublishObject implements ActionIfc {
				...
				if (scs.size() == 0) {
					...
					if (res == JOptionPane.YES_OPTION) {
						...
						for (int i = 0; i < pTarget.length; i++) {
							if (i != 0)
								sb.append(' ');
							sb.append(pTarget[i].getObjectId());
						}
						sb.append('\'');
						for (Iterator i = scs.iterator(); i.hasNext();) {
							String id = (String) i.next();
							publishingObject(clone.getSession(), id, sb
									.toString());
						}
						clone.disconnect();
					}
	...
	private void publishingObject(IDfSession session, String strJobId,
			String sourceObjectIds) throws DfException {
		...
		String cmd = strJobId
				+ ",WEBCACHE_PUBLISH,ARGUMENTS,S,-source_object_id "
				+ sourceObjectIds;
		// WEBCACHE_PUBLISH,ARGUMENTS,S,-full_refresh false -method_trace_level
		// 0

...
public class Creature2 extends Creature {
	...
	private static final int HEALTY_CREATE = 400;
	...
	int urgeCreate = 5;
	int urgeCreature = 0;
	boolean isKiller = true;
	int killerHealty = 0;
	int strange = 0;
	
	public Creature2(Creature2 parent ) {
		...
		if ( Math.random() > 0.9d ) {
			...
			isKiller = Math.random() > 0.3d;
			killerHealty = (int)(Math.random()*30d);
		}
		
	}
	
	public Creature2( boolean type ) {
		if ( type ) {
			for ( int i = 0; i < Surface.MAX_VALUE; i++ ) {
	...
	public void move(Surface s) {
		...
		if ( healty > HEALTY_CREATE ) {
			if ( s.getCreatureSize( newX, newY ) < 2 ) {
				s.addCreature( new Creature2( this ), newX, newY );
				healty-=200;
				isCreated = true;
			}
		}
		...
		if ( isKiller && !isCreated ) {
			...
			for ( Creature other : creatures )
				if ( ((Creature2)other).strange*other.getHealty() < strange*healty ) {
